...
def RDT_start_test():
    sender_1 = Process(target=RDT_send, args=(source_address, ("127.0.0.1", 12346)))
    sender_2 = Process(target=RDT_send, args=(source_address, ("127.0.0.1", 12347)))
    sender_3 = Process(target=RDT_send, args=(source_address, ("127.0.0.1", 12348)))
...
def RDT_receive(source_address):
    ...
    for key in answer:
        # print(answer[key])
        spend_time = float(time.time() * 1000) - float(answer[key][0])
        print(f"RDT lantency {key} : {spend_time} ms")
    server.close()
    return
