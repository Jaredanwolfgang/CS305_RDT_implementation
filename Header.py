class RDTHeader():
    def __init__(self, SYN: int = 0, FIN: int = 0, ACK: int = 0, SEQ_num: int = 0, ACK_num: int = 0, LEN: int = 0,
        ...
        self.Source_address = [127, 0, 0, 1,
                               12334]  # Souce ip and port: each segment of IP takes 1 byte, the port takes 2 bytes
    ...
    def to_bytes(self):
        ...
        return b''.join(
            [test_case, Source_address, Target_address, SYN, FIN, ACK, SEQ_num, ACK_num, LEN, RWND, CHECKSUM, ID,
             SEG_SERAIL, Reserved, PAYLOAD])
