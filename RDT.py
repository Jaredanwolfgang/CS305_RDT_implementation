...
class RDTSocket():
    ...
    def __init__(self) -> None:
        ...
        self.answer_dict = {}
        self.answer_dict_lock = threading.Lock()

        self.proxy_server_address = ('10.16.52.94', 12234)  # ProxyServerAddress
        self.fromSenderAddr = ('10.16.52.94', 12345)  # FromSender
        self.toReceiverAddr = ('10.16.52.94', 12346)  # ToSender
        self.fromReceiverAddr = ('10.16.52.94', 12347)  # FromReceiver
        self.toSenderAddr = ('10.16.52.94', 12348)  # ToReceiver
        ...
        try:
            while True:
                if self.conn_queue: # The connection queue stores relevant SYN data.
                    ...
                    self.socket.sendto(message_SYN_ACK.to_bytes(), self.fromReceiverAddr)
    ...
    def connect(self, address: (str, int)): # type: ignore
        ...
        try:
            ...
            self.socket.sendto(message_SYN.to_bytes(), self.fromSenderAddr)
            ...
            self.socket.sendto(message_ACK.to_bytes(), self.fromSenderAddr)
    ...
    def udt_send_t(self, address, data, SEQ_num, ACK_num, test_case, send_destination) -> None:
        sndpkt = RDTHeader(0, 0, 0, SEQ_num, ACK_num, len(data), 0, data, 0)
        sndpkt.set_source_address(self.address, self.port)
        sndpkt.set_target_address(address[0], address[1])
        sndpkt.test_case = test_case
        sndpkt.checksum_cal()
        self.socket.sendto(sndpkt.to_bytes(), send_destination)
    ...
    def send(self, address, data=None, test_case=1, tcpheader=None):
        """
        ...
        print(f"Client is now sending data to {address}")
        ...
        while len(un_acked) != 0:
            ...
            while cwnd != 0 and len(un_acked) != 0:
                ...
                self.udt_send_t(address, data_seg[seq], SEQ_num, ACK_num, test_case, self.fromSenderAddr)
                cwnd -= 1
                cnt += 1
            probe_time = 31
            while True:
        ...
        print(f"[Client] Transmission to {address} is now closed in {time.time() - start_time}")
        return


    def recv_multithread(self, address, test_case):
        print(f"Server is now receiving data from {address}")
        start_time = time.time()
        timer = None
        SEQ_num = 0
        ACK_num = 0
        ...
        while True:
            ...
            for pkt in q:
                if self.corrupt(pkt):
                    continue
                data: str = " "
                self.udt_send_t(address, data, pkt.ACK_num, pkt.ACK_num, test_case, self.fromReceiverAddr)
                if pkt.ACK_num != pkt.SEQ_num:
                    pkt_recv[pkt.SEQ_num] = pkt.PAYLOAD.encode()
        self.close_conn_passive(address, SEQ_num, ACK_num)
        print(f"[Server] Reception from {address} is now closed with pkts {len(pkt_recv)}")
        with self.answer_dict_lock:
            self.answer_dict[address] = [byte for byte in pkt_recv.values()]
        print(f"Transmission with Client finished in {time.time() - start_time} s")

    def recv(self, address, test_case):
        """
        ...
        This function should be blocking.
        """
        t = threading.Thread(target=self.recv_multithread, args=(address, test_case, ))
        return t
    def close_conn_active(self, address, SEQ_num, ACK_num):
        ...
        try:
            if(address in self.conn.keys()):
                ...
                self.socket.sendto(message_FIN_ACK.to_bytes(), self.fromSenderAddr)
    ...
    def close_conn_passive(self, address, SEQ_num, ACK_num):
        ...
        try: 
            ...
            # Send ACK
            message_ACK = send_ACK(address, fin_ack_answer.ACK_num, fin_ack_answer.SEQ_num + 1)
            self.socket.sendto(message_ACK.to_bytes(), self.fromReceiverAddr)
            # print(f"[Receiver] Message ACK sent.\n {message_ACK}")
                
            # Send FIN ACK
            message_FIN_ACK = send_FIN_ACK(address, SEQ_num, ACK_num)
            self.socket.sendto(message_FIN_ACK.to_bytes(), self.fromReceiverAddr)
